...
class EasyGroceries {
	...
	public function getProductsByDepartment ($department_id) {
		...
		try {
			...
			$query = "SELECT ea_product.id, upc, brand, product_name, 
            product_description, avg_price, unit, name
			FROM ea_product, ea_category
            WHERE ea_product.category_id = ea_category.id
			...
			foreach($this->dbo->query($query) as $row) {
				...
				$product_description = stripslashes($row[4]);
                $avg_price = stripslashes($row[5]);
                $unit = stripslashes($row[6]);
                $department_name = stripslashes($row[7]);
                ...
                // get the unit information for each product
                $query = "SELECT name
                FROM unit
                WHERE unit_id = '$unit'
                ORDER BY name ";
                //print("$query");
                foreach($this->dbo->query($query) as $row) {
                    $unit_name = stripslashes($row["0"]);
                }
                $product["unit_name"] = $unit_name;

                // end of getting the unit information 
